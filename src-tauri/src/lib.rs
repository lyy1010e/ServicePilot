...
impl ServicePilotBackend {
    ...
    async fn run_startup_helper_command(
        &self,
        service_id: Option<&str>,
        mut command: Command,
        error_context: &str,
    ...
    async fn startup_was_canceled(&self, service_id: &str) -> bool {
        let inner = self.inner.lock().await;
        !matches!(
    ...
    async fn log_startup_canceled(&self, service_id: &str) {
        {
            let mut inner = self.inner.lock().await;
            if let Some(runtime) = inner.runtime.get_mut(service_id) {
                runtime.status = RuntimeStatus::Stopped;
    ...
    async fn track_startup_helper_process(&self, service_id: &str, pid: u32) -> bool {
        let mut inner = self.inner.lock().await;
        let startup_active = inner
            .runtime
            .get(service_id)
    ...
    async fn untrack_startup_helper_process(&self, service_id: &str, pid: u32) {
        let mut inner = self.inner.lock().await;
    ...
    async fn scan_spring_services(&self, root_dir: String) -> BackendResult<ScanResult> {
    ...
    async fn scan_single_spring_service(&self, dir: &Path) -> Option<ScannedService> {
    ...
    async fn batch_import_services(
        &self,
...
fn extract_maven_modules(pom: &str) -> Vec<String> {
...
async fn has_spring_application_entry(dir: &Path) -> bool {
...
async fn java_file_contains_spring_entry(path: &Path) -> bool {
...
fn extract_maven_artifact_id(pom: &str) -> Option<String> {
...
async fn extract_server_port_from_dir(dir: &Path) -> Option<u16> {
...
fn extract_port_from_yaml(content: &str) -> Option<u16> {
...
fn extract_port_from_properties(content: &str) -> Option<u16> {
    for line in content.lines() {
        let trimmed = line.trim();
...
pub fn run() {
    ...
    let builder = builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            exit_app,
            shutdown,
            scan_spring_services,
            batch_import_services
        ]);
...
mod tests {
    ...
    fn frontend_package_manager_detection_uses_lockfiles() {
        let root =
            std::env::temp_dir().join(format!("service-pilot-test-{}", uuid::Uuid::new_v4()));
