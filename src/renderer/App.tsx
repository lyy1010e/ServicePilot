...
function parseProfiles(input: string): string[] {
  return input
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean);
...
function buildDefaultClasspath(workingDir: string): string {
...
function formatDuration(runtime: RuntimeState | undefined, now: number): string {
  if (!runtime) {
    return '--';
  }
...
function toggleValue(values: string[], target: string, checked: boolean): string[] {
  if (checked) {
...
function buildRuntimeSummary(snapshot: AppSnapshot) {
  const summary = {
